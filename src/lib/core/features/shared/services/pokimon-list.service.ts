...
import { NamedAPIResource, PaginatedResponse, PokimonListItem, PokemonDetail } from '../models/pokimon-list-item';
...
export class PokimonListService {
  ...
  getPokemonList(limit: number, offset: number): Observable<{
    total: number;
    pokemon: PokimonListItem[];
  }> {
    return this.http
      .get<PaginatedResponse<NamedAPIResource>>(
        `${this.baseUrl}/pokemon?limit=${limit}&offset=${offset}`
      )
      .pipe(
        switchMap((res) => {
          const requests = res.results.map((item) =>
            this.getPokemonDetails(item.url).pipe(
              map((details) => this.toListItem(item, details))
            )
          );
    ...
    getPokemonById(id: number) {
      return this.http.get<PokemonDetail>(`${this.baseUrl}/pokemon/${id}`).pipe(catchError(this.handleError));
    }
